"""Giant Squid bingo game - let the giant squid win."""
import re
...
def play(
    ...
    get_last_place: bool = False,
) -> int:
    played_numbers: List[str] = []
    for input in inputs:
        played_numbers.append(input)
        if len(played_numbers) > 4:
            # boards_to_loop = deepcopy(list_of_boards)
            boards_to_remove: List[Board] = []
...
def main():
    ...
    last_place = play(inputs, list_of_boards, True)
